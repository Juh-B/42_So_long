...
/*   Updated: 2025/01/21 16:54:34 by lhenriqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	key_hook(mlx_key_data_t key_args, void *param)
{
	...
	if (key_args.os_key == ESC)
		mlx_close_window(get_mlx()->mlx_ptr);
	else if ((key_args.key == W || key_args.os_key == UP)
		&& key_args.action != 0)
		ft_move_player(img, x, y - HEIGHT);
	else if ((key_args.key == S || key_args.os_key == DOWN)
		&& key_args.action != 0)
		ft_move_player(img, x, y + HEIGHT);
	else if ((key_args.key == A || key_args.os_key == LEFT)
		&& key_args.action != 0)
		ft_move_player(img, x - WIDTH, y);
	else if ((key_args.key == D || key_args.os_key == RIGHT)
		&& key_args.action != 0)
		ft_move_player(img, x + WIDTH, y);
}
